    ...
    class Table
      ...
      class Schema
        ...
        def initialize gapi
          ...
          gapi.schema.fields ||= []
          @gapi = gapi
          @fields = @gapi.fields.map { |g| Field.from_gapi f }
          @original_fields_json = @fields.to_json
        end

        # @private
        def changed?
          @original_fields_json == @fields.map(&:to_gapi).to_json
        end
        ...
        class Field
          ...
          def to_gapi
            @gapi.fields = @fields.map(&:to_gapi) unless @fields.nil?
            @gapi
          end
        end
